...
--Begin chat settings
  ...
  if matches[1] == 'mute' and is_owner(msg) then
	...
	--Begin chat sikuser
		if matches[1] == "sikuser" and is_momod(msg) then
		local chat_id = msg.to.id
		local hash = "sik_user"..chat_id
		local user_id = ""
			if type(msg.reply_id) ~= "nil" then
				local receiver = get_receiver(msg)
				local get_cmd = "sik_user"
				get_message(msg.reply_id, sik_user_callback, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == "sikuser" and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_siked_user(chat_id, user_id) then
					sik_user(chat_id, user_id)
					return "["..user_id.."] removed from the siked users list"
				else
					unsik_user(chat_id, user_id)
					return "["..user_id.."] added to the siked user list"
				end
			elseif matches[1] == "sikuser" and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "sik_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
